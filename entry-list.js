...
//   - [M] Consider use of MutationObserver instead of setInterval
//   - [M] Handle case where external source doesn't have a user
//
// Changelog:
// ----------
...
- determineType(lists, entryData, entry):
  ...
  If there is a single processing type, the method might as well return
  true/false.
...
window.EL = new (function() {
    ...
    function processOneEntry(entry, ctx = mainContext) {
        ...
        entry.ELProcessed = true; // if ELProcessed, it is skipped on next passes
    }
    ...
    function handleToggleButton(evt) {
        ...
        if (Number.isInteger(Number(data.howToFindEntry))) {
            for (var i = 0; i < Number(data.howToFindEntry); i++) {
    ...
    this.init = function(ctx, loadlistsAtStartup = false) {
        ...
        if (isEntryPage || ctx.pageType) {
            ...
            if (ctx.pageType && ctx.processPage) {
                ctx.processPage(ctx.pageType, isEntryPage);
            }
        }
    ...
    this.startProcessing = function(ctx = mainContext) {
        ...
        processTimer = setInterval(self.processAllEntries
    ...
    this.addToggleEventOnClick = function(button, howToFindEntry
                                        , toggleList = null, toggleType = null) {
    ...
    // return the list name as generated by inLists (to be used
    // in ctx.determineType())
