...
(function() {
    ...
    tst('wait','wait 10 seconds, then log info "value"', UU.wait(10000, 'value').then(UU.li));
    tst('thenWait','log info "start", wait 15 second, log info "1234"');
    UU.li('start');
    Promise.resolve(1234).then(UU.thenWait(15000)).then(UU.li);
    ...
    UU.GM_xhR('GET', 'http://www.guugle.com', 'test dnd ' + i).then(UU.li).catch(UU.le);
    ...
    UU.GM_xhR('GET', 'http://www.google.com', 'test dnd ' + i, { 'responseType': 'document' }).then(function(resp){UU.li(resp.responseXML2.getElementsByTagName('body')[0].innerHTML);}).catch(UU.le);
